...
```
  sensorn1.zip(sensorn2)
    ...
    .subscribe(onNext = { case (temp1, temp2) =>
      ...
      println(s"Lectura: Sensor1: $temp1°C, Sensor2: $temp2°C, Diferencia: ${(temp1 - temp2).abs} °C")

      //Calculamos la diferencia absoluta y verificamos si es mayor a 5
      if ((temp1 - temp2).abs > 5) {
        println(s"🚨 ALERTA: La diferencia es de ${(temp1 - temp2).abs}°C ")//Imprime la alerta
      }
    },
...
```
 .subscribe(
      ...
      println(s"Temperature: $tempS1°C, Humidity: $humedadS2%, Pressure: $presS3 hPa")

      // Validar Temperatura
      if (tempS1 < 20 || tempS1 > 30) {
        println(s"🚨 ALERT! Temperature out of range: $tempS1°C")
      }

      // Validar Humedad 
      if (humedadS2 < 40 || humedadS2 > 60) {
        println(s"🚨 ALERT! Humidity out of range: $humedadS2%")
      }

      // Validar Presión 
      if (presS3 < 950 || presS3 > 1050) {
        println(s"🚨 ALERT! Pressure out of range: $presS3 hPa")
      }
